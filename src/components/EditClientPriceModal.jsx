...
export default function EditClientPriceModal({ serviceId, client, onClose, onSubmit }) {
  const [price, setPrice] = useState(client.pivot.price)
  ...
  function handleSubmit(e) {
    ...
    api(`/services/${serviceId}/clients/${client.id}`, {
      method: 'POST',
      headers: {
    ...
    <Modal title="Editar precio" onClose={onClose}>
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        <label className="flex flex-col gap-1.5 text-[13px] font-semibold text-text-h">
          Cliente
          <input type="text" className={`${inputClass} text-text-muted`} value={client.name} disabled />
        </label>
        <label className="flex flex-col gap-1.5 text-[13px] font-semibold text-text-h">
          Precio
          <input
